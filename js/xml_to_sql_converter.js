...
ace.require("ace/ext/language_tools");var editorAce1=ace.edit("code1");editorAce1.focus();editorAce1.setOptions({enableBasicAutocompletion:true,enableSnippets:true,enableLiveAutocompletion:true});editorAce1.setTheme("ace/theme/monokai");editorAce1.getSession().setMode("ace/mode/xml");var editorAce2=ace.edit("code2");editorAce2.setOptions({enableBasicAutocompletion:true,enableSnippets:true,enableLiveAutocompletion:true});editorAce2.setTheme("ace/theme/monokai");editorAce2.getSession().setMode("ace/mode/sql");$("#clear").click(function(e){e.preventDefault();editorAce1.setValue("");editorAce2.setValue("");});$("#clear_code1,#clear_code2").click(function(e){e.preventDefault();if($(this).is("#clear_code1"))
...
blob=new Blob([""+data+""],{type:"text/plain;charset=utf-8"});saveAs(blob,"data.sql");});$("#max_code1").click(function(e){e.preventDefault();if(!$(this).is(".maximized"))
...
{$(this).removeClass("wrapon");editorAce1.getSession().setUseWrapMode(false);editorAce2.getSession().setUseWrapMode(false);$("#wrapmode").text("Smart Wrap");}});$(window).resize(function(e){$("#code1,#code2").css({"width":"100%"});});$("#sample").click(function(e){e.preventDefault();data="xml";editorAce1.getSession().setUseWorker(false);editorAce1.setValue("Please wait ...");$.ajax({type:"POST",url:"get-sample.php",data:{"data":data},success:function(data)
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue(data);$("#sql_insert").click();},error:function()
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue("Connection lost. Try again!");}});});$("#sql_insert").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");x2js=new X2JS();json=JSON.stringify(x2js.xml_str2json(data));data=jsonToCsv(json,",",true,false,false);csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
...
editorAce2.setValue(insert);}catch(e)
{editorAce2.setValue("Error parsing data!");}}});$("#sql_update").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");x2js=new X2JS();json=JSON.stringify(x2js.xml_str2json(data));data=jsonToCsv(json,",",true,false,false);csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
{cols[i]=cols[i].replace(/[^a-z0-9_$ ]/gi,"");cols[i]=cols[i].replace(/ /g,"_");cols[i]=(($.trim(cols[i])=="")?("column_"+i):$.trim(cols[i]));}
csvArr=csvArr.filter(function(e){return e.replace(/(\r\n|\n|\r)/gm,"")});if(csvArr[1]!=undefined)
{col_type_arr=csvArr[1].split(",");}else
...
editorAce2.setValue(update);}catch(e)
{editorAce2.setValue("Error parsing data!");}}});$("#sql_delete").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");x2js=new X2JS();json=JSON.stringify(x2js.xml_str2json(data));data=jsonToCsv(json,",",true,false,false);csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
{cols[i]=cols[i].replace(/[^a-z0-9_$ ]/gi,"");cols[i]=cols[i].replace(/ /g,"_");cols[i]=(($.trim(cols[i])=="")?("column_"+i):$.trim(cols[i]));}
csvArr=csvArr.filter(function(e){return e.replace(/(\r\n|\n|\r)/gm,"")});if(csvArr[1]!=undefined)
{col_type_arr=csvArr[1].split(",");}else
...
editorAce2.setValue(delete_sql);}catch(e)
{editorAce2.setValue("Error parsing data!");}}});themelist=ace.require("ace/ext/themelist");theme="";$(themelist.themes).each(function(key,value){theme+='<option value="'+value.name+'">'+value.caption+'</option>';});$("#themes").append(theme);$("#themes").val("monokai");$("#font_size").val("14");$("#themes,#font_size").change(function(e){setTheme();editorAce1.focus();});});