...
{editorAce1.setValue(html_beautify(data,opts));}});$("#sql_insert").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
...
{editorAce2.setValue("Error parsing data!");}}catch(e)
{editorAce2.setValue("Error converting to SQL!");}}});$("#sql_update").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{try{html=$.parseHTML("<div>"+data+"</div>");cols=[];content="";if($(html).find('table').length==0)
{editorAce2.getSession().setUseWorker(false);editorAce2.setValue("No table found in the html!");return false;}
editorAce2.getSession().setUseWorker(true);$(html).find('table').each(function(index,element){$(this).find("th").each(function(index,element){cols.push($(this).text().toLowerCase());});});$(html).find('table').each(function(index,element){$(this).find("tr").each(function(index,element){result=[];$(this).find('td').each(function(index){result.push($(this).text());});content+=result.join()+"\n";});});csv_data=$.trim(cols.join()+content);editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");data=csv_data.replace(/'/g,"\\'");csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
{cols[i]=cols[i].replace(/[^a-z0-9_$ ]/gi,"");cols[i]=cols[i].replace(/ /g,"_");cols[i]=(($.trim(cols[i])=="")?("column_"+i):$.trim(cols[i]));}
...
{editorAce2.setValue("Error parsing data!");}}catch(e)
{editorAce2.setValue("Error converting to SQL!");}}});$("#sql_delete").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{try{html=$.parseHTML("<div>"+data+"</div>");cols=[];content="";if($(html).find('table').length==0)
{editorAce2.getSession().setUseWorker(false);editorAce2.setValue("No table found in the html!");return false;}
editorAce2.getSession().setUseWorker(true);$(html).find('table').each(function(index,element){$(this).find("th").each(function(index,element){cols.push($(this).text().toLowerCase());});});$(html).find('table').each(function(index,element){$(this).find("tr").each(function(index,element){result=[];$(this).find('td').each(function(index){result.push($(this).text());});content+=result.join()+"\n";});});csv_data=$.trim(cols.join()+content);editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");data=csv_data.replace(/'/g,"\\'");csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
{cols[i]=cols[i].replace(/[^a-z0-9_$ ]/gi,"");cols[i]=cols[i].replace(/ /g,"_");cols[i]=(($.trim(cols[i])=="")?("column_"+i):$.trim(cols[i]));}
...
{editorAce2.setValue("Error parsing data!");}}catch(e)
{editorAce2.setValue("Error converting to SQL!");}}});themelist=ace.require("ace/ext/themelist");theme="";$(themelist.themes).each(function(key,value){theme+='<option value="'+value.name+'">'+value.caption+'</option>';});$("#themes").append(theme);$("#themes").val("monokai");$("#font_size").val("14");$("#themes,#font_size").change(function(e){setTheme();editorAce1.focus();});});