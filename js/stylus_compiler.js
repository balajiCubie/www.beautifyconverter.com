$(document).ready(function(e) {
    ...
    editorAce1.getSession().setMode("ace/mode/stylus");
	...
	$("#compile_stylus").click(function(e) {
        e.preventDefault();
		data = $.trim(editorAce1.getValue());
		if(data != "")
		{
			stylus(data).render(function(err, out){
        		if(err != null)
				{
					editorAce2.getSession().setUseWorker(false);
					editorAce2.setValue("Stylus parsing error!");
				}else
				{
					editorAce2.getSession().setUseWorker(true);
					out = cssbeautify($.trim(out), {
						indent : '	',
