$(document).ready(function(e) {
	...
	$("#convert").click(function(e) {
		...
		if(data != "")
		{
			...
			editorAce2.setValue(html_beautify(output, opts));
			
				data = $($.trim(editorAce2.getValue())).filter("table");
				$("div.data_tb").html(data);
				$("div.data_tb table").attr("id", "data_out_tb");
			    var pdfthemee = "";
				if($("#pdftheme").val() == "grid"){
					pdfthemee = "grid";
				}
				else if($("#pdftheme").val() == "striped"){
					pdfthemee = "striped";
				}
				else if($("#pdftheme").val() == "plain"){
					pdfthemee = "plain";
				}
				
				var linebreaks = $("#lineb").is(":checked" );
				var lin = " ";
				if(linebreaks){
					lin = 'linebreak';
				}else{
			    ...
			    doc.text(tabtitle, 40, 40);
			    var res = doc.autoTableHtmlToJson(document.getElementById("data_out_tb"));
			    doc.autoTable(res.columns, res.data,  {startY: 50, theme: pdfthemee, styles: {overflow: lin},});
