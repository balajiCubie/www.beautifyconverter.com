...
blob=new Blob([""+data+""],{type:"text/plain;charset=utf-8"});saveAs(blob,"data.php");});$("#max_code1").click(function(e){e.preventDefault();if(!$(this).is(".maximized"))
...
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue(data);$("#html-filter").click();},error:function()
...
$("#file").change(function(e){e.preventDefault();read_file(e);});$("#fz").click(function(){if($(this).is(':checked')){$('#preplace,#place,#nextplace').removeClass("hide");}else{$('#preplace,#place,#nextplace').addClass("hide");}})
$("#html-filter").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{var fhtml=true;var fjs=false;var fcss=false;var fz=false;if($('#fh').is(':checked')){fhtml=true;}else{fhtml=false;}
if($('#fj').is(':checked')){fjs=true;}else{fjs=false;}
if($('#fc').is(':checked')){fcss=true;}else{fcss=false;}
if($('#fz').is(':checked')){fz=true;}else{fz=false;}
if($('#emptyline').is(':checked')){el=true;}else{el=false;}
var s=data;if(fjs)
s=s.replace(/<\s*script[^>]*>(.|[\r\n])*?<\s*\/script[^>]*>/gi,'');if(fcss)
s=s.replace(/<\s*style[^>]*>(.|[\r\n])*?<\s*\/style[^>]*>/gi,'');if(fhtml){s=s.replace(/<\/?[^>]+>/g,'');s=s.replace(/\&[a-z]+;/gi,'');}
if(fz){s=s.replace(new RegExp($("#preplace").val(),'g'),$("#nextplace").val());}
if(el){var lines=s.split("\n");var uniquelines=[];$.each(lines,function(i,line){if(line.trim().length!=0){uniquelines.push(line);}});s=uniquelines.join("\n");var j;var right=true;var left=true;var a=s.split(/\r\n|\r|\n/);for(j=0;j<a.length;j++){if(right&&left)a[j]=a[j].trim();else if(left)a[j]=a[j].ltrim();else if(right)a[j]=a[j].rtrim();}
s=a.join("\n");}
editorAce2.setValue(s);}});function trimLines(s){}
themelist=ace.require("ace/ext/themelist");theme="";$(themelist.themes).each(function(key,value){theme+='<option value="'+value.name+'">'+value.caption+'</option>';});$("#themes").append(theme);$("#themes").val("monokai");$("#font_size").val("14");$("#themes,#font_size").change(function(e){setTheme();editorAce1.focus();});});