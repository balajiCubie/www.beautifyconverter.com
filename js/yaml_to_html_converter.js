...
{$(this).removeClass("wrapon");editorAce1.getSession().setUseWrapMode(false);editorAce2.getSession().setUseWrapMode(false);$("#wrapmode").text("Smart Wrap");}});$(window).resize(function(e){$("#code1,#code2").css({"width":"100%"});});$("#sample").click(function(e){e.preventDefault();data="yaml";editorAce1.getSession().setUseWorker(false);editorAce1.setValue("Please wait ...");$.ajax({type:"POST",url:"get-sample.php",data:{"data":data},success:function(data)
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue(data);$("#yaml_to_html").click();},error:function()
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue("Connection lost. Try again!");}});});var opts={};opts.indent_size=4;opts.indent_char=" ";opts.eol="\n";opts.indent_level=0;opts.indent_with_tabs=false;opts.preserve_newlines=true;opts.max_preserve_newlines=5;opts.jslint_happy=false;opts.space_after_anon_function=false;opts.brace_style="collapse";opts.keep_array_indentation=false;opts.keep_function_indentation=false;opts.space_before_conditional=true;opts.break_chained_methods=false;opts.eval_code=false;opts.unescape_strings=false;opts.wrap_line_length=0;opts.wrap_attributes="auto";opts.wrap_attributes_indent_size=4;opts.end_with_newline=false;$("#yaml_to_html").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());editorAce2.getSession().setUseWorker(true);if(data!="")
{try{obj=YAML.parse(data);json=JSON.stringify(obj);csv=jsonToCsv(json,",",true,false,false);lines=csv.split(/\r\n|\n\r|\n|\r/g);output="";for(i=0;i<lines.length;i++)
{output+="<tr>";col=lines[i].split(",");for(j=0;j<col.length;j++)
{output+="<td>"+col[j]+"</td>";}
output+="</tr>";}
var starthtml="<!DOCTYPE html><html><head><title>Beautifyconverter.com Yaml To HTML Converter</title></head><body><table>";var endhtml="</table></body></html>"
output=starthtml+output+endhtml;editorAce2.setValue(html_beautify(output,opts));}catch(e){errorData="";errorData=errorData+"Error : "+e['message'];if(e['parsedLine']!=undefined)
{errorData=errorData+"\n";errorData=errorData+"Line : "+e['parsedLine']+"  "
+e['snippet'];}
editorAce2.getSession().setUseWorker(false);editorAce2.setValue(errorData);}}});$("#validate").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());editorAce2.getSession().setMode("ace/mode/plain_text");editorAce2.getSession().setUseWorker(false);if(data!="")
{try{obj=YAML.parse(data);editorAce2.setValue("YAML IS VALID");}catch(e){errorData="";errorData=errorData+"Error : "+e['message'];if(e['parsedLine']!=undefined)
{errorData=errorData+"\n";errorData=errorData+"Line : "+e['parsedLine']+"  "
+e['snippet'];}
editorAce2.setValue(errorData);}}});$("#preview").click(function(e){e.preventDefault();if($.trim(editorAce2.getValue())=="")
{$("#convert").click();}
data=$.trim(editorAce2.getValue());try{found=$(data).filter("table").is("table");if(found)
{data=$(data).filter("table");$("div.data_tb").html(data);$("div.data_tb table").attr("id","data_out_tb");if($("#data_out_tb tr").length==0)
{editorAce2.setValue("No rows in the table!");$("div.data_tb").hide();return false;}
$("#data_out_tb").addClass("table table-bordered table-striped table-hover");$("#data_out_tb").css({"background-color":"#FFF","width":"100%"});$("#data_out_tb").find("tr:eq(0)").addClass("tr_first");if($("#data_out_tb tr").length>12)
{$("div.data_tb").css({"height":"800px"});}else
{$("div.data_tb").css({"height":"auto"});}
$("div.data_tb").show();$("html, body").animate({scrollTop:$("div.data_tb").offset().top},500);}else
{editorAce2.setValue("Table not found!");$("div.data_tb").hide();}}catch(e)
{editorAce2.setValue("Table not found!");$("div.data_tb").hide();}});themelist=ace.require("ace/ext/themelist");theme="";$(themelist.themes).each(function(key,value){theme+='<option value="'+value.name+'">'+value.caption+'</option>';});$("#themes").append(theme);$("#themes").val("monokai");$("#font_size").val("14");$("#themes,#font_size").change(function(e){setTheme();editorAce1.focus();});});