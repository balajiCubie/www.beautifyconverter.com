...
{editorAce1.setValue("Unable to load file from url!");}});}});$("#wrapmode").click(function(e){e.preventDefault();if(!$(this).is(".wrapon"))
...
{$(this).removeClass("wrapon");editorAce1.getSession().setUseWrapMode(false);editorAce2.getSession().setUseWrapMode(false);$("#wrapmode").text("Smart Wrap");}});$("#browse").click(function(e){e.preventDefault();$("#file").click();});function read_file(e)
...
blob=new Blob([""+data+""],{type:"text/plain;charset=utf-8"});saveAs(blob,"data.yaml");});$("#max_code1").click(function(e){e.preventDefault();if(!$(this).is(".maximized"))
...
{$(this).removeClass("maximized");$("div.div_code2").removeClass("col-md-12").addClass("col-md-5");$("#code2").css({"width":"100%"});$("div.buttons_div,div.div_code1,div.div_code2").show();$(this).find("span").removeClass("glyphicon-resize-small").addClass("glyphicon-fullscreen");$("#zclip-ZeroClipboardMovie_2").css({"left":($("#copy_data2").position().left-5)+"px","top":$("#copy_data2").position().top+"px"});$(this).attr("title","maximize");editorAce1.resize();editorAce2.resize();}});$("#code1").resizable({handles:'s',alsoResize:"#code2",resize:function(event,ui){editorAce1.resize();editorAce2.resize();}});$("#code2").resizable({handles:'s',alsoResize:"#code1",resize:function(event,ui){editorAce1.resize();editorAce2.resize();}});$(window).resize(function(e){$("#code1,#code2").css({"width":"100%"});});$("#sample").click(function(e){e.preventDefault();data="sql1";editorAce1.getSession().setUseWorker(false);editorAce1.setValue("Please wait ...");$.ajax({type:"POST",url:"get-sample.php",data:{"data":data},success:function(data)
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue(data);$("#convert").click();},error:function()
{editorAce1.getSession().setUseWorker(true);editorAce1.setValue("Connection lost. Try again!");}});});function jsonToText(json){var retText='';if(typeof json=="object"){if(json instanceof Array){$.each(json,function(key,val){retText+=jsonToText(val);retText+="----------\n";});}
else{$.each(json,function(key,val){if(typeof val=="object"){retText+=key+"\n"+jsonToText(val);}
else{retText+=key+" "+jsonToText(val);}});}}
else{retText+=json.toString()+"\n";}
return retText;}
$("#convert").click(function(e){e.preventDefault();data=$.trim(editorAce1.getValue());if(data!="")
{try{if(data.toLowerCase().search("create")==-1)
{editorAce2.setValue("You are missing CREATE statement.");return false;}else if(data.toLowerCase().search("select")==-1)
{editorAce2.setValue("You are missing SELECT statement.");return false;}
db=new SQL.Database();res=db.exec(data);jsonData="[";$.each(res,function(i,value){$.each(value.values,function(j,value1){jsonArr="{";$.each(value1,function(k,data){jsonArr+='"'+value.columns[k]+'"  :'+'"'+data
+'"';if(value1.length!=(k+1))
{jsonArr+=",";}});jsonArr+="}";if(value.values.length!=(j+1)){jsonArr+=",";}
jsonData+=jsonArr;});if(res.length!=(i+1))
{jsonData+=",";}});jsonData+="]";text=JSON.parse(jsonData);editorAce2.setValue(jsonToText(text));}catch(e){errorData="";errorData=errorData+"Error : "+e['message'];editorAce2.setValue(errorData);}}});themelist=ace.require("ace/ext/themelist");theme="";$(themelist.themes).each(function(key,value){theme+='<option value="'+value.name+'">'+value.caption+'</option>';});$("#themes").append(theme);$("#themes").val("monokai");$("#font_size").val("14");$("#themes,#font_size").change(function(e){setTheme();editorAce1.focus();});});