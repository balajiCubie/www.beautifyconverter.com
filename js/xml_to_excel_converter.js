	...
	$("#xml_to_excel").click(function(e) {
		...
		if(data != "")
		{
			try{
				...
				var starthtml = "<!DOCTYPE html><html><head><title>Beautifyconverter.com XML To HTML Converter</title></head><body><table>";
				var endhtml = "</table></body></html>"
				output1 = starthtml + output + endhtml;
				//editorAce2.setValue(html_beautify(output1, opts));
				html2excel(output1);
				editorAce2.setValue(csv);
			}catch(e){
				editorAce2.setValue("Invalid XML");
			}
		}
    });
    $("#xml_to_csv").click(function(e) {
        e.preventDefault();
		data = $.trim(editorAce1.getValue());
		if(data != "")
		{
