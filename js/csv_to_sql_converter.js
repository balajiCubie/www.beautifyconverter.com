...
{editorAce2.setValue("Please wait while parsing data.");try{tb_name=(($.trim($("#table_name").val())=="")?"table_name":$.trim($("#table_name").val()));tb_name=tb_name.replace(/[^a-z0-9_$ ]/gi,"");tb_name=tb_name.replace(/ /g,"_");data=data.replace(/'/g,"\\'");csvArr=data.split(/\r\n|\n\r|\n|\r/g);cols=csvArr[0].split(",");cols_length=cols.length;col_type_arr=new Array();for(i=0;i<cols_length;i++)
