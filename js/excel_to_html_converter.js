...
{editorAce1.setValue("No rows in the table!");$("div.data_tb").hide();return false;}
...
{output+="<td>"+col[j]+"</td>";}
output+="</tr>";}
output="<table>"+output+"</table>";editorAce1.setValue(html_beautify(output,opts));}catch(e){if(name.split(".").pop()=="csv")
{lines=data.split(/\r\n|\n\r|\n|\r/g);output="";for(i=0;i<lines.length;i++)
{output+="<tr>";col=lines[i].split(",");for(j=0;j<col.length;j++)
{output+="<td>"+col[j]+"</td>";}
output+="</tr>";}
output="<table>"+output+"</table>";editorAce1.setValue(html_beautify(output,opts));}else
